...
try {
    ...
    $historyFilters = [];
    if (!empty($_GET['search']))
        $historyFilters['search'] = $_GET['search'];
...
<div class="page-header">
    <div style="display:flex;align-items:center;justify-content:space-between;width:100%;">
        <div style="display:flex;align-items:center;gap:1rem;">
            <div
                style="width:48px;height:48px;background:var(--danger-light);border:1px solid var(--border-color);border-radius:12px;display:flex;align-items:center;justify-content:center;color:var(--danger);">
        ...
        <div class="page-actions" style="margin:0;">
            ...
            <button type="button" class="btn btn-secondary"
                onclick="document.getElementById('compliance-history-panel').classList.add('open')">
...
<div class="stats-grid">
    ...
    <a href="<?= buildStatUrl('critical') ?>" class="stat-card-link">
        <div
            class="stat-card <?= (isset($_GET['status']) && $_GET['status'] === 'critical') ? 'active-critical' : '' ?>">
...
<div class="card mt-8">
        ...
        <form method="GET" id="filterForm"
            ...
            <div style="position:relative;flex:1;min-width:200px;">
                <i data-lucide="search"
                    style="position:absolute;left:10px;top:50%;transform:translateY(-50%);width:15px;height:15px;color:var(--text-muted);pointer-events:none;"></i>
                <input type="text" name="search" id="searchInput" class="form-control"
                    style="padding-left:34px;width:100%;" placeholder="Search plate, model, or ref..."
            ...
            <div style="display:flex;gap:.5rem;flex-shrink:0;">
                ...
                <?php if (!empty($_GET['search']) || !empty($_GET['status']) || !empty($_GET['type'])): ?>
                    <a href="index.php" class="btn btn-ghost btn-sm" title="Clear Filters"><i data-lucide="rotate-ccw"
    ...
    <div class="table-container" style="border:none;">
        <table>
            <thead>
                <tr>
                    <th>
                        <a href="<?= buildSortUrl('v.plate_number', $currentSortBy, $currentSortOrder) ?>"
                            class="sortable-header">Vehicle Target
                            <?= getSortIcon('v.plate_number', $currentSortBy, $currentSortOrder) ?></a>
                    </th>
                    <th>
                        <a href="<?= buildSortUrl('c.compliance_type', $currentSortBy, $currentSortOrder) ?>"
                            class="sortable-header">Document Type
                            <?= getSortIcon('c.compliance_type', $currentSortBy, $currentSortOrder) ?></a>
                    </th>
                    <th>
                        <a href="<?= buildSortUrl('c.document_number', $currentSortBy, $currentSortOrder) ?>"
                            class="sortable-header">Reference #
                            <?= getSortIcon('c.document_number', $currentSortBy, $currentSortOrder) ?></a>
                    </th>
                    <th>
                        <a href="<?= buildSortUrl('c.expiry_date', $currentSortBy, $currentSortOrder) ?>"
                            class="sortable-header">Expiry Horizon
                            <?= getSortIcon('c.expiry_date', $currentSortBy, $currentSortOrder) ?></a>
                    </th>
                    <th>
                        <a href="<?= buildSortUrl('c.status', $currentSortBy, $currentSortOrder) ?>"
                            class="sortable-header">State
                            <?= getSortIcon('c.status', $currentSortBy, $currentSortOrder) ?></a>
                    </th>
                ...
                <?php if (!empty($items)):
                    foreach ($items as $item):
                        ...
                        <tr <?= $isPending ? '' : ($isBreached ? 'style="background:var(--danger-light);"' : ($isCritical ? 'style="background:var(--warning-light);"' : '')) ?>>
                            <td>
                                <a href="../asset-tracking/vehicle-details.php?id=<?= $item['vehicle_id'] ?>"
                                    style="font-weight:600;display:flex;align-items:center;gap:4px;color:inherit;">
                            ...
                            <td style="font-family:monospace;font-weight:600;">
                                <?= htmlspecialchars($item['document_number'] ?? 'N/A') ?></td>
                            <td
                                style="font-weight:600; color:<?= $isPending ? 'var(--text-muted)' : ($isBreached ? 'var(--danger)' : 'var(--text-main)') ?>;">
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="empty-state">
                                ...
                                <p style="margin-bottom:1rem;">Your instruments are perfectly optimized. No compliance tasks
    ...
    <?php if ($totalPages > 1): ?>
        <div
            style="padding:1rem 1.5rem;border-top:1px solid var(--border-color);display:flex;gap:.5rem;justify-content:center;flex-wrap:wrap;">
...
<div id="compliance-history-panel"
    ...
    <div
        style="display:flex;align-items:center;justify-content:space-between;padding:1.25rem 1.5rem;border-bottom:1px solid var(--border-color);">
        <h2 style="margin:0;font-size:1.1rem;display:flex;align-items:center;gap:.5rem;font-weight:800;">
            ...
            <?php if (!empty($_GET['search'])): ?>
                <span class="badge badge-secondary" style="font-size:0.7rem; margin-left:10px;">Filtered:
                    <?= htmlspecialchars($_GET['search']) ?></span>
            <?php endif; ?>
        </h2>
                ...
                <div style="display:flex;align-items:flex-start;justify-content:space-between;padding:1.25rem;margin-bottom:1rem;border:1px solid var(--border-color);border-radius:12px;background:var(--bg-surface,#fff);box-shadow:0 1px 3px rgba(0,0,0,.04);transition:transform 0.2s, box-shadow 0.2s;"
                    ...
                    <div style="flex-grow:1;padding-right:1rem;">
                        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                            ...
                            <span class="badge <?= $hStatusCls ?>"
                                style="font-size:0.65rem;text-transform:uppercase;padding:2px 6px;"><?= $h['status'] ?></span>
                        </div>
                        <div style="font-size:.95rem;font-weight:700;margin-bottom:6px;color:var(--text-main);">
                            <?= htmlspecialchars($h['brand'] . ' ' . $h['model']) ?>
                        </div>
                        <div
                            style="font-size:.8rem;color:var(--text-muted);margin-bottom:10px;display:flex;align-items:center;gap:6px;">
                            <i data-lucide="hash" style="width:12px;height:12px;"></i>
                            <span
                                style="font-family:monospace;background:var(--secondary-50);padding:2px 6px;border-radius:4px;color:var(--secondary-700);font-weight:700;">
                        ...
                        <div
                            style="font-size:.75rem;color:var(--text-muted);display:flex;align-items:center;gap:4px;font-weight:600;">
                    ...
                    <div style="display:flex;flex-direction:column;gap:.5rem;min-width:80px;text-align:right;">
                        <a href="instrument-view.php?id=<?= $h['record_id'] ?>" class="btn btn-ghost btn-sm"
                            style="font-weight:700;font-size:0.75rem;">Inspect</a>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
